...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.mpm2.toml)")
...
func initConfig() {
	...
	if cfgFile != "" {
		...
		// Search config in home directory with name ".mpm2" (without extension).
	...
	showconfig("Conf after reading .mpm2.toml")
	...
	showconfig("Conf after programatic get host name")
